...
- **Scroll-reveal animations**: `src/hooks/useScrollReveal.ts` (GSAP + `ScrollTrigger` + `@gsap/react`) is used in almost every section component. Call it once per animated subtree (e.g. `const scope = useScrollReveal<HTMLElement>()`), attach the returned ref to that subtree's outermost element, then mark children with `data-reveal` (fades/slides in on scroll into view) or `data-reveal-immediate` (plays on mount, cascading by DOM order) instead of writing bespoke animation code.
